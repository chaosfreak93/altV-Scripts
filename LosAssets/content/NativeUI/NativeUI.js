...
import Control from '@LosAssets/content/NativeUI/enums/Control';
import HudColor from '@LosAssets/content/NativeUI/enums/HudColor';
import ChangeDirection from '@LosAssets/content/NativeUI/enums/ChangeDirection';
import UIMenuCheckboxItem from "@LosAssets/content/NativeUI/items/UIMenuCheckboxItem";
import UIMenuItem from "@LosAssets/content/NativeUI/items/UIMenuItem";
import UIMenuListItem from "@LosAssets/content/NativeUI/items/UIMenuListItem";
import UIMenuAutoListItem from "@LosAssets/content/NativeUI/items/UIMenuAutoListItem";
import UIMenuSliderItem from "@LosAssets/content/NativeUI/items/UIMenuSliderItem";
import Container from "@LosAssets/content/NativeUI/modules/Container";
import ItemsCollection from "@LosAssets/content/NativeUI/modules/ItemsCollection";
import ListItem from "@LosAssets/content/NativeUI/modules/ListItem";
import ResRectangle from "@LosAssets/content/NativeUI/modules/ResRectangle";
import ResText from "@LosAssets/content/NativeUI/modules/ResText";
import Sprite from "@LosAssets/content/NativeUI/modules/Sprite";
import Color from "@LosAssets/content/NativeUI/utils/Color";
import Common from "@LosAssets/content/NativeUI/utils/Common";
import LiteEvent from "@LosAssets/content/NativeUI/utils/LiteEvent";
...
import InstructionalButton from '@LosAssets/content/NativeUI/modules/InstructionalButton';
import Scaleform from '@LosAssets/content/NativeUI/utils/Scaleform';
import BigMessage from '@LosAssets/content/NativeUI/modules/BigMessage';
import MidsizedMessage from '@LosAssets/content/NativeUI/modules/MidsizedMessage';
import UIMenuDynamicListItem from '@LosAssets/content/NativeUI/items/UIMenuDynamicListItem';
...
export default class NativeUI {
    ...
    IsMouseInListItemArrows(item, topLeft, safezone) {
        native.beginTextCommandGetWidth("jamyfafi");
        native.addTextComponentSubstringPlayerName(item.Text);
        ...
        const labelSize = native.endTextCommandGetWidth(false) * width * 0.35;
    ...
    ProcessMouse() {
        ...
        if (Screen.IsMouseInBounds(new Point(0, 0), new Size(30, 1080)) && this._mouseEdgeEnabled) {
            native.setGameplayCamRelativeHeading(native.getGameplayCamRelativeHeading() + 5.0);
            native.setMouseCursorSprite(6);
        ...
        for (let i = this._minItem; i <= limit; i++) {
            ...
            if (Screen.IsMouseInBounds(new Point(xpos, ypos), new Size(xsize, ysize))) {
                ...
                if (native.isControlJustReleased(0, 24) || native.isDisabledControlJustReleased(0, 24))
        ...
        if (Screen.IsMouseInBounds(new Point(extraX, extraY), new Size(431 + this.WidthOffset, 18))) {
            ...
            if (native.isControlJustPressed(0, 24) || native.isDisabledControlJustPressed(0, 24)) {
                if (this.MenuItems.length > this._maxItemsOnScreen + 1)
                    this.GoUpOverflow();
                else
                    this.GoUp();
    ...
    ProcessControl() {
        ...
        if (native.isControlJustReleased(0, 177)) {
            this.GoBack();
        }
        if (this.MenuItems.length == 0)
            return;
        if (native.isControlPressed(0, 172) && this._lastUpDownNavigation + 120 < Date.now()) {
            ...
            if (this.MenuItems.length > this._maxItemsOnScreen + 1)
                ...
                this.GoUp();
            this.UpdateScaleform();
        } else if (native.isControlJustReleased(0, 172)) {
            this._lastUpDownNavigation = 0;
        } else if (native.isControlPressed(0, 173) && this._lastUpDownNavigation + 120 < Date.now()) {
            this._lastUpDownNavigation = Date.now();
            if (this.MenuItems.length > this._maxItemsOnScreen + 1)
                this.GoDownOverflow();
            else
                this.GoDown();
            this.UpdateScaleform();
        } else if (native.isControlJustReleased(0, 173)) {
            this._lastUpDownNavigation = 0;
        } else if (native.isControlPressed(0, 174) && this._lastLeftRightNavigation + 100 < Date.now()) {
            this._lastLeftRightNavigation = Date.now();
            this.GoLeft();
        } else if (native.isControlJustReleased(0, 174)) {
            this._lastLeftRightNavigation = 0;
        } else if (native.isControlPressed(0, 175) && this._lastLeftRightNavigation + 100 < Date.now()) {
            this._lastLeftRightNavigation = Date.now();
            this.GoRight();
        } else if (native.isControlJustReleased(0, 175)) {
            this._lastLeftRightNavigation = 0;
        } else if (native.isControlJustReleased(0, 201)) {
            this.SelectItem();
        }
    }
    ...
    UpdateScaleform() {
        ...
        this._instructionalButtonsScaleform.callFunction("SET_DATA_SLOT", 0, native.getControlInstructionalButton(2, Control.PhoneSelect, false), this.SelectTextLocalized);
        this._instructionalButtonsScaleform.callFunction("SET_DATA_SLOT", 1, native.getControlInstructionalButton(2, Control.PhoneCancel, false), this.BackTextLocalized);
        let count = 2;
    ...
    render() {
        ...
        if (this._buttonsEnabled) {
            native.drawScaleformMovieFullscreen(this._instructionalButtonsScaleform.handle, 255, 255, 255, 255, 0);
